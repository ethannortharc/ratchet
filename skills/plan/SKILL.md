...
- `.ratchet/test-suite/` should exist (generated by spec skill). If missing, warn and suggest re-running `/ratchet:spec`.
...
Load `.ratchet/test-suite/manifest.yaml` to understand what tests exist for each constraint. WP acceptance criteria should reference these test files directly.
...
work_packages:
  - id: string              # wp-00, wp-01, ...
    ...
    test_files: [string]     # References to .ratchet/test-suite/ files for this WP
    ratchet:
      budget: int
    ...
    acceptance:
      - id: string          # Maps to Intent Spec constraint
        ...
        check: string
        test_file: string    # Reference to specific test in .ratchet/test-suite/
...
1. **Reference pre-generated test suite.** Don't generate new tests — use what `/ratchet:spec` already created in `.ratchet/test-suite/`.
